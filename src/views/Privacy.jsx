...
export default function Privacy() {
  ...
  if (lang === 'en') {
    return (
      <div className={styles.page}>
        ...
        <ul>
          <li><strong>Pi Network authentication:</strong> your Pi username and Pi user identifier (uid). We do not receive your email or phone number through Pi sign-in.</li>
          ...
          <li><strong>Pi payments (tips):</strong> when you send a Pi tip, the payment identifier and transaction id are processed to approve and complete the payment. We do not store your Pi wallet keys or balance.</li>
        ...
        <p>
          ...
          calculate EP and streaks, and complete Pi tips you choose to send). We do
          not sell or share personal data with third parties for advertising.
        </p>

        <h2>3. Where it lives</h2>
        <p>
          Data is stored on Supabase (Postgres + Storage), hosted in the AWS
          ap-northeast-2 (Seoul) region. Transit and storage are encrypted. Pi
          authentication and Pi payments are processed by Pi Network under its own
          privacy policy.
        </p>
  ...
  return (
    <div className={styles.page}>
      ...
      <h2>1. 수집하는 정보</h2>
      <ul>
        <li><strong>Pi 네트워크 인증:</strong> Pi 사용자명(username)과 Pi 사용자 식별자(uid). Pi 로그인으로는 이메일·전화번호를 받지 않습니다.</li>
        ...
        <li><strong>Pi 결제(후원):</strong> Pi 후원 시 결제 식별자와 트랜잭션 ID를 승인·완료 처리에 사용합니다. Pi 지갑 키나 잔액은 저장하지 않습니다.</li>
        <li><strong>익명 분석:</strong> Vercel Analytics 집계 페이지뷰 (개인 식별 정보 없음)</li>
      </ul>

      <h2>2. 이용 목적</h2>
      <p>
        서비스 운영(활동 표시, 친구 피드 전달, EP·연속일 계산, 사용자가 보내는
        Pi 후원 완료 처리). 광고 목적으로 제3자에게 개인정보를 판매하거나
        제공하지 않습니다.
      </p>
      ...
      <p>
        ...
        리전에서 호스팅됩니다. 전송 구간과 저장 구간 모두 암호화됩니다. Pi 인증과
        Pi 결제는 Pi Network가 자체 개인정보처리방침에 따라 처리합니다.
      </p>
